...
export const DEFAULT_BOOTNODE_PEER_ID = "12D3KooWEyoppNCUx8Yx66oV9fJnriXwCcXwDDUA2kj6vnc6iDEp";
export const DEFAULT_BOOTNODE_DOMAIN = "bootnode";
export const DEFAULT_CHAIN_SPEC_COMMAND = 'polkadot build-spec --chain {{chainName}} --disable-default-bootnode --raw > /cfg/{{chainName}}.json';
export const DEFAULT_GENESIS_GENERATE_COMMAND = "/usr/local/bin/adder-collator export-genesis-state > /cfg/genesis-state";
export const DEFAULT_WASM_GENERATE_COMMAND = "/usr/local/bin/adder-collator export-genesis-wasm > /cfg/genesis-wasm";
...
export function generateNetworkSpec(config: LaunchConfig): ComputedNetwork {
  ...
  for (const node of config.relaychain.nodes) {
    ...
    const bootnodes = ( node.bootnodes && node.bootnodes.length ) ?
      node.bootnodes :
  ...
  if (config.parachains && config.parachains.length) {
    ...
    if( config.relaychain.chain_spec_path ) {
      const chainSpecPath = resolve(process.cwd(), config.relaychain.chain_spec_path);
      ...
      DEFAULT_CHAIN_SPEC_COMMAND.replace(new RegExp("{{chainName}}", "g"), chainName);
    }

    for(const parachain of config.parachains ) {
      let computedStatePath, computedStateCommand, computedWasmPath, computedWasmCommand;
      const bootnodes = ( parachain.bootnodes && parachain.bootnodes.length ) ?
        parachain.bootnodes :
        [`/dns/${DEFAULT_BOOTNODE_DOMAIN}/tcp/30333/p2p/${DEFAULT_BOOTNODE_PEER_ID}`];

      if( parachain.genesis_state_path) {
        const genesisStatePath = resolve(process.cwd(), parachain.genesis_state_path);
        if (!fs.existsSync(genesisStatePath)) {
          console.error("Genesis spec provided does not exist: ", genesisStatePath);
          process.exit();
        } else {
      ...
      if( parachain.genesis_wasm_path) {
        const genesisWasmPath = resolve(process.cwd(), parachain.genesis_wasm_path);
        if (!fs.existsSync(genesisWasmPath)) {
          console.error("Genesis spec provided does not exist: ", genesisWasmPath);
          process.exit();
        } else {
      ...
      parachainSetup = {
        ...
        ...(computedWasmCommand) ? { genesisWasmGenerator: computedWasmCommand }: {},
        ...(computedStatePath) ? { genesisStatePath: computedStatePath } : {},
        ...(computedStateCommand) ? {genesisStateGenerator: computedStateCommand } : {}
      };
