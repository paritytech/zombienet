...
export async function series( functionsThatGeneratePromisesThatRunInSeries: any[], concurrency = 1 ) {
  ...
  return new Promise( ( resolve, reject ) => {
          const next = (result?:any) => {
                  ...
                  if( functionsThatGeneratePromisesThatRunInSeries.length ) {
                          while( concurrentPromises.length < concurrency && functionsThatGeneratePromisesThatRunInSeries.length ) {
