...
const getVersion = async (image: string, command: string): Promise<SubstrateCliArgsVersion> => {
  const client = getClient();
  const fullCmd = `${command} --help | grep ws-port`;
  const logs = (await client.runCommand(["-c", fullCmd],{allowFail: true})).stdout;
  debug("logs:");
  debug(logs);

  return logs.includes("--ws-port <PORT>") ? SubstrateCliArgsVersion.V0 : SubstrateCliArgsVersion.V1;
}
...
export const setSubstrateCliArdsVersion = async(network: ComputedNetwork) => {
    ...
    network.relaychain.nodes.reduce((memo, node) => {
      ...
      if(! memo.has(uniq_image_cmd)) memo.set(uniq_image_cmd,{ image: node.image, command: node.command });
      return memo;
    }, imgCmdMap);
    ...
    network.parachains.reduce((memo, parachain) => {
      for(const collator of parachain.collators) {
        ...
        if(! memo.has(uniq_image_cmd)) memo.set(uniq_image_cmd,{ image: collator.image, command: collator.command });
      }
      return memo;
    }, imgCmdMap);
