...
async function addParaCustom(specPath: string, node: Node) {
  ...
  const node_index = runtimeConfig.balances.balances.findIndex((item: [string, BigInt]) => {
  ...
  const candidate_index = runtimeConfig.balances.balances.findIndex((item: [string, BigInt]) => {
    return item[0] === eth_account.address.toLowerCase();
  });

  if(candidate_index > -1) runtimeConfig.balances.balances.splice(candidate_index, 1);
