...
export async function generateParachainFiles(
  ...
  if (parachain.cumulusBased) {
    ...
    if (!(await isRawSpec(chainSpecFullPathPlain))) {
      ...
      else if (plainData.genesis.runtimeGenesisConfigPatch?.parachainInfo?.parachainId)
        plainData.genesis.runtimeGenesisConfigPatch.parachainInfo.parachainId = parachain.id;
